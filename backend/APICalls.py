...
class ProviderClient:
    ...
    def _get_cached(self, key: str, ttl: int, allow_stale: bool = False) -> Optional[ProviderResponse]:
        ...
        if self.category == "geo":
            ...
            if row:
                ...
                if age < ttl:
                    return ProviderResponse(row.conditionsJson or {}, self.providerName, "hit")
                if allow_stale and age < MAX_STALE["weather"]:
                    return ProviderResponse(row.conditionsJson or {}, self.providerName, "stale")
    ...
    def _normalizeForReturn(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.category == "weather":
            #AviationWeather wants wrapped; OpenWeatherMap wants raw
            if getattr(self, "providerName", "") == "openweathermap":
                return raw or {}
